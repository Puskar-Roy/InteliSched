...
app = firebase_admin.initialize_app(cred,name="faculty_panel")
...
def handel_faculty_details(db_id):
    # data = db.child("Faculties").child(db_id) its for real time data base
    # user_data = data.get().val() also same
    result = db.collection("Faculties").document(db_id).get()
    user_data = 0
    if result.exists:
...
# ========================================================================================================Deatails Page
    def home_page(event):
            ...
            lable= ctk.CTkLabel(frame1,text="Faculty Details",
            ...
            lable3 = ctk.CTkLabel(frame1,text="Faculty Id : "+user_data["Faculty Id"],
                                font=ctk.CTkFont(weight="bold",size=25,family="Helvetica"))
            ...
            lable6 = ctk.CTkLabel(frame1,text="Role : "+user_data["Role"],
                                font=ctk.CTkFont(weight="bold",size=25,family="Helvetica"))
...
# ---------------------------------------------------------------------------------
    ...
    def change_password():
        def update():
            if(cur_mail.get() == user_data["Password"]):
                if(new_pass.get()!= cur_mail.get()):
                    ...
                    db.collection("Faculties").document(user_data["Faculty Id"]).update({"Password":new_pass.get()})
        ...
        label2 = ctk.CTkLabel(root1,text="Enter New Password",font=ctk.CTkFont(size=15,weight="bold"))
        ...
        def update():
            if(cur_mail.get() == user_data["Email"]):
                if(new_mail.get()!= cur_mail.get()):
                    ...
                    db.collection("Faculties").document(user_data["Faculty Id"]).update({"Email":new_mail.get()})
                    messagebox.showinfo(title="Updtae",message="Update Email Successfull")
                    root.destroy()
                else:
...
# ===============================================================
    ...
    home_btn = ctk.CTkLabel(option_frame,text="Faculty Details",
    ...
    lablex2= ctk.CTkLabel(main_frame,text=user_data["Name"],
                                    font=ctk.CTkFont(weight="bold",size=30,family="Helvetica"))
...
if __name__ == "__main__":
    handel_faculty_details("202301")   



# data base faka tai parameter a akichu dite hbe    